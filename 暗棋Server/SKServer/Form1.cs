...
namespace SKServer
{
    public partial class Form1 : Form
    {
        ...
        private void chooseWhoFirst(pairPlayer pp)
        {
            ...
            startList = RandomSortList(boardList);
            String ordListStr="";
            foreach(int a in startList){
                ordListStr += a.ToString () +";";
            }
            ordListStr ="," + ordListStr .Substring (0, ordListStr .Length -1) +",";
            ...
            if (client==1){
                ...
                Send(pp, "S" + ordListStr );
                Send(pp.oppenent , "W"+ ordListStr);
            }
            else
            {
                //SocketClient2.Send(Encoding.Default.GetBytes("S"));
                //SocketClient1.Send(Encoding.Default.GetBytes("W"));
                Send(pp, "W"+ ordListStr);
                Send(pp.oppenent, "S"+ ordListStr);
